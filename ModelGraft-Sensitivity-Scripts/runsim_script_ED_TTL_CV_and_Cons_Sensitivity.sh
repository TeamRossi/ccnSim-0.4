...
cvT=$(echo ${parameters[20]})				# Coefficient of Variation (CV) threshold.
consT=$(echo ${parameters[21]})		# Consistency threshold.
...
if [[ $SimType == "TTL" ]]
	...
	if [[ $MetaCaching == "two_ttl" ]]
		then
		TcNameFileTemp=$(echo ${parameters[23]})		
...
echo "CV Threshold  =  ${cvT}"
echo "Consistency Threshold  =  ${consT}"
...
# CV Threshold
`awk -v v1="${cvtLS}" -v v2="${cvT}" '$1==v1{$5='v2'}{print $0}' ${iniFileFinal} > ${iniFileFinal}_temp.ini`
`mv ${iniFileFinal}_temp.ini ${iniFileFinal}`

# Consistency Threshold
`awk -v v1="${constLS}" -v v2="${consT}" '$1==v1{$5='v2'}{print $0}' ${iniFileFinal} > ${iniFileFinal}_temp.ini`
`mv ${iniFileFinal}_temp.ini ${iniFileFinal}`
...
# Output Strings
outString=${SimType}_T_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_RS_${ReplStr}_C_${CacheDim}_NC_${NameCacheDim}_M_${TotalCont}_Req_${TotalReq}_Lam_${Lambda}_A_${Alpha}_CT_${ClientType}_ToffMult_${Toff}_Start_${startType}_Fill_${fillType}_ChNodes_${checkNodes}_Down_${down}_CvT_${cvT}_ConsT_${consT}

tcOutString=tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Lam_${Lambda}_CvT_${cvT}_ConsT_${consT}.txt
tcOutStringNameCache=tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Lam_${Lambda}_NameCache_CvT_${cvT}_ConsT_${consT}.txt
