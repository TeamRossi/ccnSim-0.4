...
if [[ $SimType == "TTL" ]]
	...
	if [ -n "$TcFileTemp" ]					# User provided Tc file
		then
		TcFile=$TcFileTemp
		TcFileSpec=$(echo ${parameters[21]})
	else
		TcFile=${tcDirIn}/tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Alpha_${Alpha}_Lam_${Lambda}.txt
	fi

	if [[ $MetaCaching == "two_ttl" ]]
		then
		TcNameFileTemp=$(echo ${parameters[22]})		
		if [ -n "$TcNameFileTemp" ]						# User provided Tc file for Name Cache
			then
			TcNameFile=$TcNameFileTemp					
		else
			TcNameFile=${tcDirIn}/tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Alpha_${Alpha}_Lam_${Lambda}_NameCache.txt
		fi
	else
		TcNameFile="0"
	fi
...
# Output Strings
outString=${SimType}_T_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_RS_${ReplStr}_C_${CacheDim}_NC_${NameCacheDim}_M_${TotalCont}_Req_${TotalReq}_Lam_${Lambda}_A_${Alpha}_CT_${ClientType}_ToffMult_${Toff}_Start_${startType}_Fill_${fillType}_ChNodes_${checkNodes}_Down_${down}_Tc-Spec_${TcFileSpec}

tcOutString=tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Alpha_${Alpha}_Lam_${Lambda}.txt
tcOutStringNameCache=tc_${Topology}_NumCl_${NumClients}_NumRep_${NumRepos}_FS_${ForwStr}_MC_${MetaCaching}_M_${TotalCont}_R_${TotalReq}_C_${CacheDim}_Alpha_${Alpha}_Lam_${Lambda}_NameCache.txt
...
if [[ $SimType == "ED" ]]
	then
	grep "Evaluated Tc:" ${logDir}/${outString}_run\=0.out | awk '{print $6}' > ${tcDirOut}/${tcOutString}
	if [[ $MetaCaching == "two_lru" ]]
		then
		grep "NAME CACHE LRU Tc:" ${logDir}/${outString}_run\=0.out | awk '{print $8}' > ${tcDirOut}/${tcOutStringNameCache}
	fi
fi
